...
def impact_text(metric, value, t):
    if metric == "CO2":
        ...
        return f" +{min(20, round(over/50))}% de concentration perçue après amélioration de la ventilation."
    if metric == "OCCUPANCY":
        excess = max(0, value - t['occupancy_max'])
        return f"-{min(30, excess*3)}% de bruit et meilleure sécurité."
    if metric == "TEMPERATURE":
        return " amélioration du confort thermique et de la satisfaction."
    if metric == "LIGHT":
        return " baisse de la fatigue visuelle et augmente la productivité."
    if metric == "HUMIDITY":
        return " confort respiratoire et santé perçue améliorés."
    return " amélioration de l’expérience."
...
for _, row in latest.iterrows():
    ...
    if pred == 1:
        recommendations.append({
            ...
            "impact": " maintien du confort et de la productivité."
        })
        continue
...
if top_day:
    ...
    impact = f" +{max(5, int(demand_delta_pct))}% de capacité absorbée" if demand_delta_pct > 0 else "Impact prévu : meilleure répartition de la charge"
    recommendations.append({
...
if top_equipment:
    recommendations.append({
        ...
        "impact": " +0.2 à +0.4 sur l’indice de satisfaction."
    })
...
if gender_prefs:
    ...
    for g, lst in gender_prefs.items():
        ...
        recommendations.append({
            ...
            "priority": "Priorité faible",
            "impact": " meilleure adéquation offre/besoins par segment."
        })
