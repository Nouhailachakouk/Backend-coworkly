...
public class InsightsService {
    ...
    public List<SentimentDTO> getSentimentSummary() {
        ...
        for (Map<String, Object> row : rows) {
            ...
            // Critères corrigés selon seuils idéaux
            if (temp >= 20 && temp <= 28) score++;
            if (humidity >= 40 && humidity <= 60) score++;
            if (co2 > 0 && co2 < 1000) score++;
            if (light >= 300 && light <= 1000) score++;
            if (occupancy >= 1 && occupancy <= 25) score++;
